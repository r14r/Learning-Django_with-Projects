...
class ItemViewTests(TestCase):
    ...
    def test_list_view(self):
        resp = self.client.get(reverse('search_engine:list'))
        self.assertEqual(resp.status_code, 200)
        self.assertContains(resp, 'Test Item')

    def test_detail_view(self):
        resp = self.client.get(
            reverse('search_engine:detail', kwargs={'pk': self.item.pk})
        )
        self.assertEqual(resp.status_code, 200)
        self.assertContains(resp, 'Test Item')

    def test_create_requires_login(self):
        resp = self.client.get(reverse('search_engine:create'))
        self.assertNotEqual(resp.status_code, 200)

    def test_create_item(self):
        self.client.login(username='tester', password='secret123')
        resp = self.client.post(
            reverse('search_engine:create'),
    ...
    def test_delete_item(self):
        self.client.login(username='tester', password='secret123')
        self.client.post(
            reverse('search_engine:delete', kwargs={'pk': self.item.pk})
        )
        self.assertEqual(Item.objects.count(), 0)

    def test_update_requires_login(self):
        resp = self.client.get(
            reverse('search_engine:update', kwargs={'pk': self.item.pk})
        )
        self.assertNotEqual(resp.status_code, 200)
    ...
    def test_detail_view_404(self):
        resp = self.client.get(
            reverse('search_engine:detail', kwargs={'pk': 9999})
        )
